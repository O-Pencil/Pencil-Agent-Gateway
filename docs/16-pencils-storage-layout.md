...
│   │   └── …（同 default 结构）
│   │
│   ├── hanhan/                             ← 云端"领养"槽（id='hanhan' slug；displayName='韩寒'）
│   │   ├── agent.json                      ← 规范元数据（id / displayName / origin / externalId）见 §11.2.1
...
```
┌──────────────────────────────────────┐         ┌─────────────────────────────────┐
│  Cloud (Asgard)                      │         │  Local (~/.pencils/agents/hanhan/) │
│  ──────────────────                  │         │  ──────────────────              │
...
connections:
  ...
  - id: hanhan-trial                              # editor 侧 connection id（可中文，仅前端展示）
    displayName: "韩寒（试用）"
    transport: gateway-http
    baseUrl: https://asgard-api.onrender.com
    apiKey: sk-asgard-xxx
    agentModel: pencil/hanhan                     # ← Gateway 侧 agent slug，必须 ASCII
    cloudOnly: true                               # 不写本地 agentDir

  - id: hanhan-local                              # 同 templateId 的本地领养版
    displayName: "韩寒（已领养到本地）"
    transport: gateway-http
    baseUrl: http://127.0.0.1:18080               # 本地 Gateway 接管
    apiKey: pk_dev_default
    agentModel: pencil/hanhan                     # ← 同 slug，不同 agentDir
    sourceTemplate:                               # 元信息：从哪来、什么版本（对应本地 agent.json origin）
...
**领养时的本地落盘**（在 `~/.pencils/agents/<id>/`，`<id>` 来自包的 `templateId` slug 或用户在 UI 里指定的 ASCII 名）：
...
│   ├── default/                              ← CLI 单用户兼容槽
│   ├── pencil-01/                            ← 本地手工槽
│   ├── hanhan/                               ← 云端领养槽（id slug；displayName 在 agent.json）
│   └── <id>/                                 ← <id> 必须匹配 ^[a-z0-9][a-z0-9._-]{0,63}$
│       ├── agent.json                        ← 规范元数据（id/displayName/origin/externalId/tags）见 §11.2.1
...
### 11.2.1 `agent.json` —— Agent 规范元数据（Asgard 联动的关键）

> 加这一文件的动机：用户提出"Agent ID 应该是英文数字、Name 可以是中文"，且"应该有一份 Agent 的元数据存在本地，否则后期无法和 Asgard 平台联动"。

每个 `agents/<id>/agent.json` 是该 Agent 的**规范元数据 manifest**——区别于 `auth.json`（凭据）/`settings.json`（运行时偏好）/`models.json`（provider 目录），它存的是**Agent 身份与生命周期事实**。schema：
...
```jsonc
{
  ...
  "tags": ["fiction", "literary"],               // 用户分类，UI 过滤用
  "engine": "nano-pencil",                       // 该 agent 用的引擎类型
  "extensions": {}                               // 未来扩展（不要改 schema 就能加字段）
}
```

**为什么需要 `externalId`**：用户在 Asgard UI 里把 `displayName` 从"韩寒"改成"韩寒-2026"，本地 `id` 不变（**id 不可变**，否则目录得搬家），靠 `externalId` 追溯到 Asgard 那条记录。同一个 Asgard 用户在两台机器上领养"韩寒"模板——两台机器的本地 `id` 可以不同（如 `hanhan` vs `hanhan-mac`），但 `origin.asgard.externalId` 相同，云端能识别。
...
- Gateway：构造 `AgentInstance.toResponse()` 时返回 displayName / origin / tags。
- nanoPencil CLI：TUI 里"切 agent"显示 displayName 而不是 id。
- Asgard：通过 `externalId` 关联到云端 PencilAgent 表。
- editor：connections.yaml 里 `sourceTemplate` 字段直接对应 `origin.asgard.{templateId, templateVersion}`。

**与 `.PENCIL.md` 的关系**：`.PENCIL.md` 是 nanoPencil CLI 已有的 markdown 形式自描述（人读多于机器读）。`agent.json` 是机器读的规范字段。两者并存——`.PENCIL.md` 给 LLM 看（作为 system context 一部分），`agent.json` 给运维 / 集成系统看。

### 11.2.2 ID 校验入口

ID 校验必须落在**两个入口**：

1. `loadConfig()`：从 config.json 装载 agents 时，对每个 `agent.id` 跑正则。非法直接抛 `InvalidRequestError`，启动失败比静默接受更安全。
2. `AgentRegistry.register()`：从 `POST /v1/agents` 收到的 body，同样校验。

正则：`^[a-z0-9][a-z0-9._-]{0,63}$`。"韩寒"这种 displayName 走 body 的 `name` 字段（已存在）；body 的 `id` 字段是 slug。
...
7. **ID vs Name 严格分离 + 大小写敏感性**：
   ...
   - `loadConfig()` 与 `register()` 都要做 schema 校验，非法 id fail-fast。
