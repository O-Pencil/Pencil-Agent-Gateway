...
| **P1** — `AgentConfig` 加 `kind` / `parentTemplateId` / `origin`；`writeAgentMetadata` 改用 `config.kind ?? 'custom'` 与 `config.origin ?? {type:'local'}`（不再硬编码） | `0347a36` | ✅ |
| **P0-sec** — 全局 `SAFETY_GUARDRAIL`：`composeSoulPrompt()` 在用户 Soul 之后追加安全约束（API Key / 凭证 / 本服务部署 / 其他用户信息 拒答）；Soul 缺省时 guardrail 单独生效，避免 SDK 默认 system prompt fallback 绕过；部署条目限定主语为"本服务"放过通用知识 | `4b055e9` + `ed5319b` + `1c76c9f` | ✅ |
...
| `src/engine/nano-adapter.ts` | P0.5, P0-sec, G4, G8 | `cwd` 钉到 `agentDir` + `await loader.reload()`；`composeSoulPrompt()` 在 Soul 末尾追加 `SAFETY_GUARDRAIL`（Soul 缺省时单独返回 guardrail）；错误信息路径更新 |
